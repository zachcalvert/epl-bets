"""
Algorithmic odds engine — generates realistic 1X2 decimal odds for EPL matches
based on league standings (position, PPG, goal difference) and home advantage.

This replaces the external Odds API with a deterministic, zero-cost alternative.
The generated "House" line is the sole bookmaker on the site.

See docs/0036-ODDS_OVERHAUL.md for the full algorithm design.
"""

import logging
from decimal import ROUND_HALF_UP, Decimal

from django.conf import settings

from matches.models import Match, Standing

logger = logging.getLogger(__name__)

# ── Algorithm constants ───────────────────────────────────────────────────────

HOME_ADVANTAGE = Decimal("1.25")  # ~46 % EPL home-win rate historically
DRAW_BASELINE = Decimal("0.27")  # ~27 % of EPL matches end in a draw
MARGIN = Decimal("0.05")  # 5 % overround (realistic bookmaker edge)
MIN_ODDS = Decimal("1.05")
MAX_ODDS = Decimal("25.00")

# Strength blend weights
PPG_WEIGHT = Decimal("0.60")
POSITION_WEIGHT = Decimal("0.40")

# Fallback strength for teams with no standing data (mid-table equivalent)
FALLBACK_STRENGTH = Decimal("0.50")

TWO_PLACES = Decimal("0.01")


# ── Core algorithm ────────────────────────────────────────────────────────────


def _team_strength(standing):
    """Compute a 0–1 strength rating from a Standing record.

    Blends points-per-game (60 %) with a position-based rating (40 %).
    """
    played = max(standing.played, 1)
    ppg = Decimal(standing.points) / Decimal(played)
    # Normalise PPG to 0–1 (max theoretical PPG is 3.0)
    ppg_norm = ppg / Decimal("3.0")

    # Position rating: 1st → 1.0, 20th → 0.05
    pos_rating = (Decimal(21) - Decimal(standing.position)) / Decimal(20)

    return (PPG_WEIGHT * ppg_norm) + (POSITION_WEIGHT * pos_rating)


def generate_match_odds(home_standing, away_standing):
    """Generate 1X2 decimal odds for a single match.

    Parameters
    ----------
    home_standing : Standing | None
        The home team's current league standing, or None if unavailable.
    away_standing : Standing | None
        The away team's current league standing, or None if unavailable.

    Returns
    -------
    dict with keys ``home_win``, ``draw``, ``away_win`` — each a Decimal.
    """
    home_strength = _team_strength(home_standing) if home_standing else FALLBACK_STRENGTH
    away_strength = _team_strength(away_standing) if away_standing else FALLBACK_STRENGTH

    # Apply home advantage
    home_strength = home_strength * HOME_ADVANTAGE

    total = home_strength + away_strength
    if total == 0:
        # Degenerate edge case — treat as perfectly even
        total = Decimal("1")
        home_strength = Decimal("0.5")
        away_strength = Decimal("0.5")

    # Raw head-to-head probabilities (before draw carve-out)
    p_home_raw = home_strength / total
    p_away_raw = away_strength / total

    # Draw probability: closer teams → higher draw chance
    # Gap is 0 when teams are equal, approaches 1 for extreme mismatches
    gap = abs(p_home_raw - p_away_raw)
    # Scale draw baseline down as gap increases (min 8 % for huge mismatches)
    p_draw = max(
        DRAW_BASELINE * (Decimal("1") - gap),
        Decimal("0.08"),
    )

    # Redistribute remaining probability
    remaining = Decimal("1") - p_draw
    p_home = p_home_raw * remaining
    p_away = p_away_raw * remaining

    # Sanity: ensure all probabilities are positive
    p_home = max(p_home, Decimal("0.01"))
    p_draw = max(p_draw, Decimal("0.01"))
    p_away = max(p_away, Decimal("0.01"))

    # Convert to decimal odds and apply margin
    margin_factor = Decimal("1") / (Decimal("1") + MARGIN)

    home_odds = (Decimal("1") / p_home) * margin_factor
    draw_odds = (Decimal("1") / p_draw) * margin_factor
    away_odds = (Decimal("1") / p_away) * margin_factor

    # Clamp and round
    home_odds = _clamp(home_odds)
    draw_odds = _clamp(draw_odds)
    away_odds = _clamp(away_odds)

    return {
        "home_win": home_odds,
        "draw": draw_odds,
        "away_win": away_odds,
    }


def _clamp(odds):
    """Clamp odds to [MIN_ODDS, MAX_ODDS] and round to 2 decimal places."""
    clamped = max(MIN_ODDS, min(MAX_ODDS, odds))
    return clamped.quantize(TWO_PLACES, rounding=ROUND_HALF_UP)


# ── Batch generation ──────────────────────────────────────────────────────────


def generate_all_upcoming_odds(season=None):
    """Generate odds for every SCHEDULED/TIMED match in the given season.

    Returns a list of dicts:
        [{"match": Match, "home_win": Decimal, "draw": Decimal, "away_win": Decimal}, ...]
    """
    season = season or getattr(settings, "CURRENT_SEASON", "2025")

    matches = (
        Match.objects.filter(
            season=season,
            status__in=[Match.Status.SCHEDULED, Match.Status.TIMED],
        )
        .select_related("home_team", "away_team")
        .order_by("kickoff")
    )

    # Build standings lookup: team_id → Standing
    standings = Standing.objects.filter(season=season)
    standings_map = {s.team_id: s for s in standings}

    results = []
    for match in matches:
        home_standing = standings_map.get(match.home_team_id)
        away_standing = standings_map.get(match.away_team_id)

        odds = generate_match_odds(home_standing, away_standing)
        results.append({
            "match": match,
            **odds,
        })

    logger.info(
        "generate_all_upcoming_odds: generated odds for %d matches (season %s)",
        len(results),
        season,
    )
    return results
